...
def update_sparkar_optimization_settings(context):
    ...
    if context.object.modifiers.find(SPARK_DECIMATE_MODIFIER_NAME) != -1:
        ...
        ratio_modifier = context.object.modifiers[SPARK_DECIMATE_MODIFIER_NAME].ratio
...
def update_spark_decimation_ratio(self, context):
    if not is_context_valid(context):
        return
    percentage = context.active_object.sparkar_optimization.InvertedReducePercentage
    if percentage != 0:
        if context.object.modifiers.find(SPARK_DECIMATE_MODIFIER_NAME) == -1:
...
class OBJECT_OT_SparkOperator_MeshCleanUp(bpy.types.Operator, SparkOperatorsMixin):
    ...
    def _cleanup_textures(self, context):
        ...
        for texture in textures_list:
            if texture.image and texture.image.size[0] > self.TEXTURE_MAX_SIZE:
                ...
                bpy.data.images[name].scale(
                    self.TEXTURE_MAX_SIZE, self.TEXTURE_MAX_SIZE
                )
    ...
    def execute(self, context):
        ...
        bpy.ops.mesh.vert_connect_nonplanar(angle_limit=self.NONPLANAR_ANGLE_LIMIT)
...
class OBJECT_OT_SparkOperator_Decimation(bpy.types.Operator, SparkOperatorsMixin):
